...
# 3.변수와 데이터 입력
 ...
 - 인간은 키보드를 치지만, 컴퓨터는 악속된 비트열의 나열인 ACSII 형식으로 저장하며 ASCII또한 비트열 종류 중의 하나
 - 변수의 선언이란 메모리공간의 확보
 - 할당연산자 = 에는 엄청난 책임이 따른다 :: 좌항과 우항의 데이터형이 같아야한다. 다른경우 허용범위 내에서 묵시적 형변환이 일어나며 형변환시 삽입되는 데이터는 변화한다(0xff와 0x00ff는 다르다, 같지만 완벽하게 같지 않다.)
 - 형변환시의 데이터 변환은 뒤에서 자세히 다루도록 한다.
 - 형 변환이 필요한 경우, 묵시적으로 하지말고 코드상에 명확하게 명시적으로 변환 해 주기를 권함
